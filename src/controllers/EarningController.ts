...
const index = async (args, hasToken) => {
  ...
  let isSameUserOrAdm = hasToken._id == wallet.user || hasToken.role === 'ADM';
  if (!isSameUserOrAdm) throw new Error('User Unauthorized');

  const accEarningsByYear = getSumByUnicProp(wallet.earning, 'year', 'amount');

  return getArraySortByParams(accEarningsByYear, 'year');
};

const show = async (args, hasToken) => {
  if (hasToken.role == 'USER') throw new Error('User Unauthorized');
...
const store = async (args, hasToken) => {
  let wallet = await Wallet.findById(args.walletID).populate('earning');
  if (!wallet) throw new Error('Wallet Not Found');
...
const update = async (args, hasToken) => {
  if (hasToken.role == 'USER') throw new Error('User Unauthorized');
...
const count = async (args, hasToken) => {
